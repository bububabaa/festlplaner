...
$dom = file_get_html('http://www.laessigeparty.at/events', false);

//collect all user’s reviews into an array
$answer = array();
if(!empty($dom)) {
$divClass = $title = "";$i = 0;
foreach($dom->find(".event_entry") as $divClass) {
//title
foreach($divClass->find("h2") as $title ) {
$answer[$i]['h2'] = $title->plaintext;
}
//ipl-ratings-bar
foreach($divClass->find(".content") as $ipl_ratings_bar ) {
$answer[$i]['h3'] = trim($ipl_ratings_bar->plaintext);
}
